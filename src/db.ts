...
// Seed Projects (from constants.ts)
...
if (projectsCount.count === 0) {
  ...
  const insert = db.prepare(`
    ...
    VALUES (@id, @title, @location, @type, @status, @image, @description, @price, @unitsLeft)
  `);
...
if (testimonialsCount.count === 0) {
  ...
  const insert = db.prepare(`
    ...
    VALUES (@quote, @text, @author, @role, @location, @image, @rating)
  `);

  const insertMany = db.transaction((testimonials) => {
    for (const testimonial of testimonials) insert.run(testimonial);
  });
